---
title: 'ESM 206 Lab 5'
author: "Allison Horst, edited by ACG"
date: "10/25/2019"
...
# read in the lobster data within the "data" folder
...
## dives happen before lobster season begins
## year 2012 - 2018

## "IVEE" = IV reef
## "NAPL" = Naples
## "AQUE" = Arroyo Quemado reef (north of Refugio)
## "CARP" = Carp reef
## "MOHK" = Mohawk reef (west mesa)
...
# this makes it so that each lobster has its own row (this is called tidy format) ######
...
ggplot(lobster_tidy, aes(x = site, y = size_mm)) +
  geom_jitter(aes(color = site),
              width = 0.2, 
              alpha = 0.3) +
              labs(title = "Lobster Size at Five Reefs in Santa Baraba County (2012 - 2018)",
              x = "Site",
              y = "Size (mm)") 

ggsave(here::here("figures", "lobster_jitter.png"))

# Violin plot: 
ggplot(lobster_tidy, aes(x = site, y = size_mm)) +
  geom_violin(aes(color = site), alpha = 0.3) +
              labs(title = "Lobster Size at Five Reefs in Santa Baraba County (2012 - 2018)",
              x = "Site",
              y = "Size (mm)") 

ggsave(here::here("figures", "lobster_violin.png"))

# Histogram: NOTE, scale on x-axis varies making comparison accross reefs harder
...
ggsave(here::here("figures", "lobster_facet_hist.png"))

# Quantile-quantile plot: 
ggplot(lobster_tidy, aes(sample = size_mm)) +
  geom_qq(size = 0.5, alpha = 0.3) +
  facet_wrap(~site)
...
# this will change the class of the "date_new" column to class "Date"
# the alternative is to separate year / month / day into separate columns
...
A really cool thing about that? When we use `lubridate::month()`, it's auto stored as an ordered factor (so we don't need to `fct_relevel()`). 
...
# count() function does 4 things, 1) it groups by,
# 2) summarizes, 
# and then 3) finds length 
# and then at the end 4) ungroups data)
...
# will determine type of test to do based on the number of vector args that are given to it
t.test(ivee_lobsters) 
# A one sample t-test p-value tells us whether the sample values 
...
ggplot(data = lobster_ysite, aes(x = obs_year, y = site)) +
  ...
  labs(title = "Lobster Count by Reef Site & Year (2011 - 2018)",
           x = "Year",
           y = "Site")

ggsave(here::here("figures", "lobster_heat_map.png"))
```
